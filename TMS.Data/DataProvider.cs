  ...
  public abstract class DataProvider<T> : IDataProvider<T> where T : class
  {
    ...
    private void ApplyIndexes()
    {
      ...
      if (!Collection.IndexExists(indexes.ToArray()))
        Collection.CreateIndex(indexes.ToArray());
    ...
    public virtual IQueryable<T> Search(string searchString)
    {
      ...
      {
        if(propertyInfo.PropertyType.UnderlyingSystemType.Name.Contains("IEnumerable"))
          continue;
        ...
        if (pi.IsEnum)
        {
          IEnumerable<string> enumerable = Enum.GetNames(pi).Where(x => x.ToUpper().Contains(searchString.ToUpper()));
        ...
        {
          queryFields.Add(propertyInfo.Name == "Id"
            ? Query.Matches("_id", searchString)
            : Query.Matches(propertyInfo.Name, string.Format("/.*{0}.*/gi", searchString)));
        }
        else
        {
    ...
    {
      BaseEntity baseEntity = Get(Id).Single() as BaseEntity;
